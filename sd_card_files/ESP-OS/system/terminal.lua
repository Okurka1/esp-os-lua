...
function terminal.cmd_help()
  ...
  print("║  " .. a.highlight("ansi") .. " <on/off> - Toggle ANSI colors           ║")
...
function terminal.cmd_ansi(args)
  local a = get_ansi()
  
  if not args then
    local status = config.ansi_enabled and "ENABLED" or "DISABLED"
    print(a.info("\n[ANSI] Current status: " .. status))
    print("Usage: ansi <on|off>")
    return
  end
  ...
  if arg == "on" then
    ...
    local ok = sd.write("/ESP-OS/config/system.lua", 
      "config = {\n" ..
      "  device_name = " .. string.format("%q", cfg.device_name) .. ",\n" ..
      "  version = " .. string.format("%q", cfg.version) .. ",\n" ..
      "  language = " .. string.format("%q", cfg.language) .. ",\n" ..
      "  debug = " .. tostring(cfg.debug) .. ",\n" ..
      "  auto_reconnect = " .. tostring(cfg.auto_reconnect) .. ",\n" ..
      "  serial_baud = " .. tostring(cfg.serial_baud) .. ",\n" ..
      "  sd_cs_pin = " .. tostring(cfg.sd_cs_pin) .. ",\n" ..
      "  ansi_enabled = true\n" ..
      "}\n\nreturn config\n")
    
    print(a.success("✓ ANSI colors ENABLED"))
  elseif arg == "off" then
    ...
    print("[INFO] ANSI colors DISABLED")
  else
    print(a.error("[ERROR] Invalid argument. Use: ansi <on|off>"))
  end
end
